...
const validaUsuario = [
  ...
  check('tipo')
    ...
    .isIn(['Admin', 'Cliente']).withMessage('O tipo informado deve ser Admin, Cliente'),
  check('avatar')
    .default('https://ui-avatars.com/api/?background=3700B3&color=FFFFFF&name=Hamburgueria')
...
router.get('/id/:id', async (req, res) => {  
  try {
    db.collection(nomeCollection).find({ '_id': { $eq: ObjectId(req.params.id) } }, {
      projection: { senha: false }
    }).limit(1).toArray((err, docs) => {      if (!err) {
